namespace CSharpModule
{
    ...
    class Program
    {
        ...
        static async void OnTimedEvent(object sender, System.Timers.ElapsedEventArgs e, ModuleClient deviceClient)
        {
            ...
            try
            {
                string data = new DataHelper().GetData();

                Console.WriteLine("primary data ：" + data);
                ...
                var msg = client.PostAsync("http://13.66.231.122:3001/score", content).Result;

                string result = msg.Content.ReadAsStringAsync().Result;

                //sendMessage.Properties.Add("MessageType", "Alert");

                var sendMessage = new Message(System.Text.Encoding.Default.GetBytes(result));
                ...
                await deviceClient.SendEventAsync("output1", sendMessage);
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Exception message:" + exm);
            }
